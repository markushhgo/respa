...
from tkusers.providers.turku.provider import TurkuProvider
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        changed = False
        try:
            app = SocialApp.objects.get(provider=TurkuProvider.id)
        except SocialApp.DoesNotExist:
            app = SocialApp(provider=TurkuProvider.id)
            self.stdout.write(self.style.SUCCESS('Creating new SocialApp'))

        if not app.name:
            app.name = 'Turun kaupungin työntekijät'
            changed = True
