...
def get_update_url(resource):
    return '%s%s/update/' % (reverse('resource-list'), resource.pk)
...
def user_with_permissions():
    ...
    perm_view = Permission.objects.get(codename='view_resource', content_type=content_type)
    perm_add = Permission.objects.get(codename='add_resource', content_type=content_type)
    perm_change = Permission.objects.get(codename='change_resource', content_type=content_type)
    perm_del = Permission.objects.get(codename='delete_resource', content_type=content_type)
...
def test_reservable_in_advance_fields(api_client, resource_in_unit, test_unit, detail_url):
    ...
    before = timezone.localtime(timezone.now()).replace(hour=0, minute=0, second=0, microsecond=0) + datetime.timedelta(days=11)
    assert response.data['reservable_before'] == before
